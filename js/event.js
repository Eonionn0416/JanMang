...
async function submitEvent() {
  ...
  try {
    ...
    if (!eventDate) throw new Error("Date를 입력해 주세요.");
    if (!description) throw new Error("Description을 입력해 주세요.");

    const profile = await getCurrentUserProfile(user.uid);
    const attachments = await uploadAttachments(attachmentFiles, `posts/event/${user.uid}`, title);
    const primary = attachments[0] || { name: "", url: "", type: "", size: 0, downloadName: "" };
    const imageAttachment = attachments.find((item) => String(item.type || "").startsWith("image/"));
    ...
    const postData = {
      ...
      eventDate,
      eventLocation,
      attendees: getSelectedAttendeesPayload(),
      attendeeUids: getSelectedAttendeesPayload().map((row) => row.uid),
      attachmentName: primary.name,
      attachmentSize: primary.size,
      attachmentType: primary.type,
      attachmentUrl: primary.url,
      attachmentDownloadName: primary.downloadName || "",
      attachments,

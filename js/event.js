...
function renderSelectedFileList(files, previewId, emptyText = "No file selected") {
  ...
  box.innerHTML = files.map((file, index) => `
    <div class="file-selection-item">
      ...
      <button type="button" class="file-selection-remove" data-event-remove-target="${previewId}" data-event-remove-index="${index}" aria-label="${escapeHtml(file.name)} 삭제">삭제</button>
    </div>
  `).join("");
}
